...
#define DEFAULT_QSIZE (3+_min(d,1000))
		  ...
		  int **nb)
{
  ...
  x2 = y2 = 0;
  for (i=0; i<t.deg; i++) {
    x1 = _min(x1, t.branch[i].x);
    y1 = _min(y1, t.branch[i].y);
    x2 = _max(x2, t.branch[i].x);
    y2 = _max(y2, t.branch[i].y);
  }

  threshold_x = (x2 - x1)/4;
  threshold_y = (y2 - y1)/4;
  threshold_x = _min(threshold_x, longest);
  threshold_y = _min(threshold_y, longest);
		...
		DTYPE cx, DTYPE cy)
{
  ...
  threshold_x = _min(threshold_x, max_len);
  threshold_y = _min(threshold_y, max_len);
